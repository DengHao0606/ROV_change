...
class StereoCamera(object):
    ...
    def device_parameter_config(self, device=0, stream_format=cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), size=(3840, 1080), fps=30):
        """
    ...
    def rectification_init(self):
        ...
        self.map1_left, self.map2_left = cv2.initUndistortRectifyMap(
            self.camera_matrix_left, self.dist_coeffs_left, self.R1, self.camera_matrix_left, self.half_size, cv2.CV_32FC1)
        self.map1_right, self.map2_right = cv2.initUndistortRectifyMap(
            self.camera_matrix_right, self.dist_coeffs_right, self.R2, self.camera_matrix_right, self.half_size, cv2.CV_32FC1)
    ...
    #def rectifyImage(self, image_left, image_right=None):
    def rectifyImage(self, image_left):
        """
        校正图像
        @param image_left:输入左侧图像
        @param image_right:输入右侧图像
    ...
    def InitRectifyImage(self, image_init):
        """
        校正图像
        @param image_left:输入左侧图像
        @param image_right:输入右侧图像
        """
        rectified_init = cv2.remap(
            image_init, self.map1_right, self.map2_right, cv2.INTER_AREA)
        return rectified_init
        #rectified_right = cv2.remap(
        #    image_right, self.map1_right, self.map2_right, cv2.INTER_AREA)
        #return rectified_left, rectified_right
