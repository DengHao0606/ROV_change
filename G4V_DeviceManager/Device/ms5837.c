...
inline void Ms5837Temperature(float *p)
{
    *p = MS5837.temperture;
}
void PackSensorData(float temp, float depth, uint8_t *buf)
{
    /* 温度数据（4字节，精度0.01℃）*/
    uint32_t temp_int = (uint32_t)(temp * 100); 
    buf[0] = (temp_int >> 24) & 0xFF;
    buf[1] = (temp_int >> 16) & 0xFF;
    buf[2] = (temp_int >> 8) & 0xFF;
    buf[3] = temp_int & 0xFF;
    
    /* 深度数据（4字节，精度0.01米）*/
    uint32_t depth_int = (uint32_t)(depth * 100);
    buf[4] = (depth_int >> 24) & 0xFF;
    buf[5] = (depth_int >> 16) & 0xFF;
    buf[6] = (depth_int >> 8) & 0xFF;
    buf[7] = depth_int & 0xFF;
}
